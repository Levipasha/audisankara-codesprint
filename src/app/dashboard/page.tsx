      ...
      {user.paymentStatus !== 'paid' && (
        <div className="min-h-[70vh] flex items-center justify-center px-4">
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            ...
            ) : user?.teamId && (user?.role === 'team-leader' || user?.teamRole === 'leader') ? (
              <div className="text-center space-y-2 mb-7">
                <div className="inline-flex items-center gap-2 py-1.5 px-4 bg-purple-50 border border-purple-200 text-purple-700 text-[10px] font-extrabold uppercase rounded-full tracking-wider">
                  <span className="h-2 w-2 rounded-full bg-purple-500 animate-pulse"></span>
                  Team Payment Required
                </div>
                <h1 className="text-xl font-extrabold text-slate-900">Complete Team Registration Payment</h1>
                <p className="text-xs text-slate-500 leading-relaxed max-w-sm mx-auto">
                  Pay <strong>₹{((teamDetails?.members?.filter((m: any) => m.paymentStatus !== 'paid')?.length) || 5) * 399}</strong> online to activate registration for all {teamDetails?.members?.length || 5} members in your team.
                </p>
              </div>
            ) : (
              <div className="text-center space-y-2 mb-7">
                ...
                >
                  ...
                  <span>
                    {dashUtrLoading 
                      ? 'Processing...' 
                      : user?.teamId && (user?.role === 'team-leader' || user?.teamRole === 'leader')
                        ? `Pay ₹${((teamDetails?.members?.filter((m: any) => m.paymentStatus !== 'paid')?.length) || 5) * 399} Online Now`
                        : 'Pay ₹399 Online Now'}
                  </span>
                </button>
                {dashUtrError && (
